# 工作流优化验收档案

本文件是跨任务、跨线程共享的工作流优化记录。只记录已经通过测试和真实任务证据验证的优化；想法、待验证方案和单次临时修复不得写成“已生效”。本文件采用追加式维护，历史条目不得覆盖。

## 落档门禁

每次优化只有同时满足以下条件后才能标记为 `validated`：

1. 问题有可复现证据或真实任务数据。
2. 代码改动有对应回归测试，且相关测试和全仓测试通过。
3. 至少完成一次真实任务只读基准、真实交付验证或等价端到端验收。
4. 写明适用范围、不适用范围、回滚边界和剩余风险。
5. 更新 `AGENTS.md`、相关专项工作流文档和本档案，使新线程不依赖旧线程上下文也能执行。

## 条目模板

```markdown
## YYYY-MM-DD 优化名称

- 状态：validated | rolled_back | superseded
- 代码版本：<commit/branch>
- 触发问题：<真实问题和证据>
- 实施改动：<流程、脚本、门禁>
- 验收证据：<测试命令、测试数量、真实任务指标>
- 生效范围：<哪些任务默认使用>
- 回滚边界：<如何关闭或回到旧路径>
- 剩余风险：<尚未覆盖的边界>
- 必读文档：<相关文档路径>
```

## 2026-09-07 定向修复的姓名快照、话轮证据与已知语义回归

- 状态：`validated`，限定为显式姓名快照、证据传输、精确语境豁免及已知误译回归；不是模型全部语义能力改善证明。
- 代码版本：`main/cb74fa9` 上本地工作区改动；用户明确要求只做本地修复，暂不提交、不同步，未push。
- 触发问题：同表批准的新姓名未纳入外部术语约束；审计缺少目标先行词；自然代词回指会被新姓名硬规则误报；模型多次将英语习语的担忧对象错译仍判ACCEPT。
- 实施改动：增加显式姓名行到私有JSON快照的入口，记录来源哈希并阻断冲突/缺语种/越界；pack和cache-lint共用快照。显式对白场景自动补齐前后源文及当前目标话轮，签名包含证据，单语言请求只携带该语言目标。单格term_waivers绑定源文、译文和语境。对白审计增加目标盲读、语义字段校验与checkpoint隔离；已知法语担忧对象错位以源条件化窄规则阻断，并在审计和缓存双处复核。
- 验收证据：新增测试先复现缺失快照加载、场景证据签名不变、接受缺语义证据等失败；全仓302 passed、77 subtests passed，既有69项质量fixture通过，新增4组语义好坏例由测试另行驱动，git diff --check通过。真实定向任务复核123格，修改96格；82个原确认问题全部修正，含48处错名；537行/4,296目标格最终cache-lint与readback为0，实际修改集合等于黄底集合，原件哈希不变。
- 真实模型验证：最终3个已知坏建议均回退，2个正确对照保留；其中2个性别问题由模型回退，法语习语由确定性回归兜底。未经回归规则时模型仍会误判，不把新增盲读或格式字段声称为语义自动闭环。
- 生效范围：维护源Agent执行入口。已有工作台镜像和旧交付不自动更新；复杂长场景超过48轮只给前后各8轮并标记不完整，未知人物关系仍须人工补证。
- 回滚边界：可撤销新增快照/场景证据/盲读/窄规则，但不能通过全局关闭姓名QA、接受缺证据修改或把正确代词当姓名变体绕过错误。词形豁免只属于已核实的一格。
- 剩余风险：自然语言未知错误仍可能漏检，盲读每个含对白修改的审计批次增加一次调用；6个场景或人物信息不足项保留待证，不列为确认错误。未做Studio同步或backend验证，未重跑全表语义评分。
- 必读文档：`docs/INDEPENDENT_TRANSLATION_QUALITY_EVALUATION.md`、`AGENTS.md`、`docs/workflow-execution-thread-handoff.md`、`docs/LARGE_TEXT_MULTILINGUAL_WORKFLOW_V2.md`

## 2026-09-07 独立评估后的角色约束与审计证据传递

- 状态：`validated`，限定为输入契约、确定性角色约束及回归验证；不表示语义错误已经消除。
- 代码版本：`main` 的 `cb74fa9` 基线上的本地未提交改动；用户要求不提交、不 push，尚未同步 Studio 执行副本。
- 触发问题：真实八语成品的术语表使用“角色”分类，而大文本加载器仅识别“角色名”等标签，强名称约束为 0；二次审计调用链又只传 review_key、语言、建议和理由，没有原译、源文和语境。独立评估确认了深校引入的性别、习语退化。
- 实施改动：补齐精确角色分类别名，保留普通亲属称谓的语境弹性；英语主源不补回英语已省略的中文姓名。主控按可信 review_rows 给 auditor 补齐源模式、原译、源文、上下文、token、术语和风险标签；reviewer 自报的 current/context 不能覆盖这些证据。加强性别/称谓证据要求，并隔离新版审校和审计 checkpoint。新增独立评估规范，要求同批批准姓名先纳入临时术语快照、对白按完整话轮复核、缺陷与偏好分开统计。
- 验收证据：角色别名测试先出现 8 个预期失败；审计调用链测试先复现缺少 current 的 KeyError，修复后通过。全仓 `py -3 -X utf8 -m pytest -q` 为 293 passed、77 subtests passed；既有质量 fixture 69 项通过；`git diff --check` 通过。真实 537 行、4,296 格只读回放启用 31 条专名约束，当前术语范围误报 0；回放一处历史错名准确检出 1 个 term_missing。正式成品、源表、术语表与 QA 的哈希不变。
- 语义验证边界：三条已知退化做真实模型回放，性别两条分别 REVISE / REVERT，但法语习语一条仍错误 ACCEPT。只能说明证据传递缺陷已修复，不能宣称模型语义回归全通过，也不能以 646 个修改之类的工作量代替质量收益。
- 生效范围：维护源大文本 pack/cache-lint 和 run_deep_proofread 的下一次执行；现有交付与缓存未改。源模式、术语元数据及 checkpoint 变化会使旧快照需要重新核对。
- 回滚边界：按本次差异撤回分类、匹配和审计补证据改动；不得通过清空术语或把猜测性性别修改改成自动接受来规避失败。
- 剩余风险：同批新姓名未在外部术语表时，仍须按规范先合并临时名称基线；说话人和关系证据不齐时模型可能仍误判。Studio 同步副本存在本次任务之前的多文件差异，本次只更新入口说明，未把无关历史改动全量镜像。
- 必读文档：`docs/INDEPENDENT_TRANSLATION_QUALITY_EVALUATION.md`、`AGENTS.md`、`docs/workflow-execution-thread-handoff.md`、`docs/LARGE_TEXT_MULTILINGUAL_WORKFLOW_V2.md`

## 2026-09-02 小批量历史语言表精确检索

- 状态：`validated`
- 代码版本：本次提交，基于 `main`
- 触发问题：真实 6 条、9 语言邮件任务为了确认历史官方句式，最初尝试直接导入 1.4 MB 完整语言表并出现 30 秒超时；该行为容易被误解为重新翻译或校对整份语言包，也没有必要。
- 实施改动：新增 `scripts/run_history_lookup.py` 和 `utils/history_lookup.py`。小批量任务先加载术语表，再用当前中文原文做精确查询；历史 workbook 使用 `openpyxl read_only=True` 流式读取，只取中文列与请求语言列，全部命中后立即停止打开后续历史文件；未命中不做模糊或相似句扩展。同步更新 `AGENTS.md`、`工作流说明.md` 和线程 handoff，明确禁止小任务导入、渲染、翻译、校对或另存整份历史语言表。
- 验收证据：新增 5 项单元测试，覆盖源文去重、指定语言列、中文简称表头、拒绝标点近似匹配、全部命中后短路后续文件；真实 6 行邮件表提取为 5 条唯一中文，对 4,273 行历史语言资产进行九语精确只读查询耗时 0.662 秒，精确命中 0 后正常结束。全仓 `py -3 -X utf8 -m pytest -q` 为 290 passed、64 subtests passed；CLI help、语法编译和 `git diff --check` 通过。
- 生效范围：当前任务唯一中文不超过 200 条，且历史已验收译文仅用于精确复用的 XLSX 小批量翻译任务。
- 回滚边界：不传历史 workbook 即保持术语表加模型翻译；用户明确要求全量历史审计或任务满足大文本 V2 条件时仍走对应完整流程。不得回退为发现完整语言表后默认整本导入。
- 剩余风险：没有精确命中的文本不会获得相似句参考；如项目确实需要相似句证据，应由用户明确要求或使用项目专用句子模板，不能在小批量默认流程中静默扩大检索范围。
- 必读文档：`AGENTS.md`、`docs/workflow-execution-thread-handoff.md`、`工作流说明.md`

## 2026-08-25 结构化长文本程序托管与失败项局部修复

- 状态：`validated`
- 代码版本：工作区待提交，分支 `agent/harden-large-text-terminology-gates`
- 触发问题：真实七语任务的西语初译曾在 44 行中产生 210 个结构 blocker，集中于 3 条带大量标签/JSON 的长文本；另外 11 条只有章节序号或已知术语中文残留。旧流程把整段结构交给模型保留和重建，并在少量失败时整批重试，既慢又可能推翻已通过译文。
- 实施改动：新增程序托管模板，将有效 JSON 或含至少 3 个技术标签的中文源按 JSON 路径和标签边界拆成纯文本槽；翻译、reviewer 和 auditor 只处理纯文本，主控按原路径和精确标签序列重建。初译后新增保守本地残留修复，只处理该行命中术语和章节序号；其余残留继续由 `cache-lint` 阻断。翻译和深校 checkpoint 版本升级，避免错误复用旧整段结果。
- 验收证据：新增回归覆盖模型请求不含结构、JSON 形状与标签序列精确重建、已通过单元格不变、仅修已知残留、深校与审计不接触结构，以及 Excel 准备到交付读回的等价端到端流程；定向 40 项通过，全仓 `py -3 -X utf8 -m pytest -q` 为 281 passed、64 subtests passed，`git diff --check` 通过。
- 生效范围：大文本 V2 中 `source_mode=cn` 且源文为有效 JSON，或含至少 3 个 `<@n>`/HTML 类技术标签的行；普通文本、英文主源和历史精确复用路径不变。
- 回滚边界：删除结构化模板分流与本地残留修复调用可回到旧路径；不得在新路径失败时降级为让模型整段保留/重建结构，也不得重译已通过单元格。
- 剩余风险：无效 JSON 且技术标签少于 3 个的短文本仍走普通路径；本地残留修复只识别术语命中和章节序号，其他中文正文会继续 hard fail，需要人工确定纯文本修复内容。
- 必读文档：`docs/LARGE_TEXT_MULTILINGUAL_WORKFLOW_V2.md`、`docs/workflow-execution-thread-handoff.md`

## 2026-08-24 深校抽样与失败恢复收口

- 状态：`validated`
- 代码版本：本次提交，分支 `agent/harden-large-text-terminology-gates`
- 触发问题：真实金手指英语任务共 217 行，API 最后一个长 JSON 分包截断后通过缓存补齐完成翻译、6 包全量深校、final cache-lint 和成品读回，但 manifest 仍停留在 `api_translate_failed`；同时代码中的 `sampled` 策略虽然声明“高风险全量 + 低风险 10%”，执行函数实际仍审全部唯一文本。
- 实施改动：`sampled` 按长文本、受保护 token 密集、术语密集和显式 `risk_flags` 判定高风险，并用稳定 review signature 抽取 10% 低风险唯一文本；风险标签进入签名和审校包。runner 新增 `reconcile`，只有 final cache、final cache-lint、深校摘要、apply-dry-run、非空交付目录和 readback 全部验证通过时才允许把恢复任务收口为 `complete`，并保存原失败状态、原因、证据路径和 `recovery_retro.json`。
- 验收证据：新测试先复现 `sampled` 审查 21/21 条、显式风险标签丢失和恢复接口不存在，再修复通过；源仓库全量 `py -3 -X utf8 -m pytest -q` 为 277 passed、64 subtests passed。真实金手指任务恢复前为 `api_translate_failed`，执行 `reconcile` 后状态 `complete`、未完成阶段 0、恢复证据 6 项、recovery retro 状态 `complete`；原交付仍保持 217 条、final cache-lint hard blocker 0、readback hard blocker 0。
- 生效范围：所有使用大文本 V2 `sampled/full` 深校的本地 workbook/DOCX 任务，以及 API 异常后依靠已验证缓存继续完成的恢复任务。
- 回滚边界：使用 `full` 可继续全量审校；不调用 `reconcile` 时保持原失败状态。不得手工把 manifest 改为完成，也不得在任一门禁缺失或失败时强制收口。
- 剩余风险：稳定 10% 抽样用于低风险唯一文本，不代表全量语言质量证明；用户要求逐句、逐行、完整或深度全量校对时必须继续使用 `full`。`reconcile` 只验证已有证据，不重新执行语义审校。
- 必读文档：`docs/LARGE_TEXT_MULTILINGUAL_WORKFLOW_V2.md`、`docs/workflow-execution-thread-handoff.md`

## 2026-08-11 在线术语快照与缓存命中重算门禁

- 状态：`validated`
- 代码版本：随本次 `main` 流程门禁修复提交发布
- 触发问题：金手指 33 行七语交付漏用主角术语“双生魔偶 → Clockwork Twins”。旧任务复用了仅 2176 条的本地术语表，而在线术语表 revision 512 已有 2387 个有效行；同时飞书导出的 XLSX 把 worksheet dimension 低报为 `A1`，只读加载器将整表误判为单单元格，导致新快照仍得到 0 个术语命中。旧 `cache-lint` 又只检查缓存内已有且标为 required/strict 的命中，无法发现缺失命中。
- 实施改动：术语 pack 加载器在遇到可疑 `A1` 维度时重置维度再扫描；读取分类列，将主角、角色名、人物名、英雄名、怪物名、BOSS、NPC 标为强制术语；`cache-lint` 新增 `--term-base`，用本次术语快照重新计算命中并阻断缓存快照漂移，pipeline 初检和终检均传入 term base。
- 验收证据：3 个回归测试先分别复现分类未标强制、缓存漏词未阻断、XLSX 维度低报导致 0 命中，再修复为通过；项目全量 274 项、部署 harness 全量 258 项测试通过。真实任务重算后 33 行中 28 行命中 95 个术语，审查 665 个术语×语言组合，准确检出 21 个硬错误；修正后 cache hard blocker 0、成品读回 hard blocker 0、21 个预期改单元格与缓存逐格一致、ID/CN 和其余单元格不变。
- 生效范围：所有使用大文本 pack/cache-lint 的在线术语表与本地术语表校对任务。
- 回滚边界：不得回退为只信任缓存 `term_hits` 或在线任务默认复用旧本地术语表；若项目明确允许角色名自由变体，应在术语表分类或 accepted variants 中显式声明，不得全局关闭强术语门禁。
- 剩余风险：普通动词、属性和非命名实体仍允许词形与语境变体，确定性门禁只阻断强术语；其余语义一致性仍需逐句审校和二次纠偏。
- 必读文档：`docs/LARGE_TEXT_MULTILINGUAL_WORKFLOW_V2.md`、`docs/DEEP_LINE_PROOFREADING_SUBAGENT_WORKFLOW.md`

## 2026-08-10 大文本尾部空行与占位符增量门禁

- 状态：`validated`
- 代码版本：随本次 `main` 流程门禁修复提交发布
- 触发问题：金手指真实 33 行七语深校任务的源表预设到第 1318 行，旧 `readback-gate` 把尾部 1284 个空白样式行当成有效内容，产生 `1284 × 7 = 8988` 个假 `blank_target_cell`；独立复核同时发现 ID 17 英语凭空新增 `<@6>-<@8>` 与 `<color>/<u>` 标签，而旧 `TOKEN_RE` 不识别 `<@n>`，缓存门禁错误放行。
- 实施改动：`readback-gate` 只跳过 ID 与源文同时为空的预设行，有 ID 或源文的真实行仍检查空译文；自动受保护 token 增加 `<@n>`，对目标译文新增的 `<@n>` 一律阻断，源文完全没有富文本标签时新增标签同样阻断，同时保留已有富文本源允许补闭合标签的兼容行为。
- 验收证据：新增测试先分别稳定复现 `196` 个尾部假错误和 `<@n>` 增删 `0` 告警，再实现后通过；真实最终缓存在加固门禁下先准确报出 7 个 ID 17 英语新增 token，修正后 33 行、231 个目标单元格 hard blocker 0，本地缓存与成品逐格一致；飞书 revision `474 -> 476`，在线回读 33 行、231 个译文格、99 个元数据格，不一致 0。
- 生效范围：所有使用大文本 V2 `cache-lint` 和 `readback-gate` 的 XLSX 多语言任务。
- 回滚边界：不得恢复为扫描所有预设空行或只检查 token 丢失；若项目确实允许新增富文本，必须在源文已有富文本结构或专用结构规则中显式处理，不得全局关闭新增 token 门禁。
- 剩余风险：当前新增标签检查在源文已有任一富文本标签时保持兼容，不做严格标签多重集相等；复杂富文本仍需深校 reviewer 与最终逐格读回共同兜底。
- 必读文档：`docs/LARGE_TEXT_MULTILINGUAL_WORKFLOW_V2.md`、`docs/DEEP_LINE_PROOFREADING_SUBAGENT_WORKFLOW.md`

## 2026-07-29 飞书长表离线优先与深校断点加速

- 状态：`validated`
- 代码版本：工作区待提交，基于 `main`
- 触发问题：真实飞书前端长表任务包含 3,372 行、7 个目标语言和 23,604 个深校单元。初译耗时 3,964.734 秒；旧深校批次同时承载多语言，响应易截断，且只按整批复用 checkpoint，调整批大小会重审。现场还发现 5 个重复审校进程共享同一 checkpoint，造成重复调用；飞书导出的 XLSX worksheet dimension 错标为 `A1`，`openpyxl` 只读模式误判缺少语言列。
- 实施改动：飞书长表固定为“整表导出并记录 revision/hash -> 本地翻译/深校/QA/成品读回 -> 写前 revision 门禁 -> 指定范围分块回填 -> 在线逐格读回”；深校改为单语言小批、受控并发，翻译与深校新增独立 `batch-size/workers` 参数；review checkpoint 按 `review_key + lang` 复用，改变批大小只补缺失单元；增加单任务 `proofread.lock`，拒绝重复主进程；二次审计同样按 worker 并发；reviewer 返回 `KEEP` 但省略重复译文时使用当前译文补齐，`FIX` 为空仍阻断；XLSX 精确写回和读回改用普通加载，兼容 underreported worksheet dimension。
- 验收证据：金手指真实任务最终深校建议 3,713 项、审计回退 65 项、保留修改 3,539 个单元；缓存 hard blocker 0，本地成品与缓存逐格一致；飞书 revision 438 -> 450，在线读回 3,372 行、30,348 个 A:I 单元格，不一致 0。地狱 SLG 真实任务 491 行、380 条唯一文本，43 行复用历史译文、222 行命中术语，深校保留 78 个单元修改，处理 17 个占用英语列的合并区域；飞书 revision 387 -> 410，在线读回 1,964 个 A/B/C/E 单元格，不一致 0。新增回归覆盖单语言批次、跨批大小单元续跑、重复进程拦截、并发审计、`KEEP` 空 suggested 补齐和 worksheet dimension 异常；全量 `python -m unittest discover -s tests -p 'test_*.py'` 共 262 项通过，语法编译、CLI help 和 `git diff --check` 通过。
- 生效范围：飞书/在线表格中的大文本、多语言、全量逐行深校任务，以及所有使用大文本 V2 精确 XLSX 写回的任务。
- 回滚边界：本地文件小任务可继续直接使用原入口；关闭深校时不启动 proofread 阶段。不得回滚为边翻译边写飞书、跳过 revision 门禁、多个进程共享同一任务目录，或仅检查在线单元格非空。
- 剩余风险：API 总耗时仍受模型延迟和供应端限流影响；默认 `30 × 8 workers` 是稳定起点，不保证所有供应端都适用。在线表在最终回填期间仍可能被他人修改，因此每批必须记录 revision，发现非本任务漂移立即停止。
- 必读文档：`docs/LARGE_TEXT_MULTILINGUAL_WORKFLOW_V2.md`、`docs/workflow-execution-thread-handoff.md`

## 2026-07-28 建筑名移动端地图 UI 精简策略

- 状态：validated
- 代码版本：工作区待提交
- 触发问题：真实移动端 SLG 建筑名复核中，76 条英语正式名平均 15.33 字符，建筑标签与等级、阶级同时挤在主城地图上；普通术语流程没有建筑名类型，较长功能短语不会进入专名复核。
- 实施改动：新增显式 `ui_building_name` 分类；英语正式名采用 2 个核心词 / 18 字符软预算，有独立地图标签时目标为 14 字符；等级和阶级优先拆成 UI 徽标。超预算生成 `building_name_compactness_watch`，交给 AI/人工按语义复核，不机械截词。
- 验收证据：真实任务 76 条提案的正式名平均降至 12.14 字符、地图标签平均 9.55 字符；`python -m unittest discover -s tests` 共 256 项通过，`python scripts\run_quality_harness.py fixtures\quality_regression.json --json` 共 69 个 fixture 通过，含建筑名好坏例；语法编译与 `git diff --check` 通过。
- 生效范围：术语表 `分类/category/type` 明确为建筑名或设施名的移动端地图、主城和建筑列表 UI；其他语言参考英语功能结构和简洁度自然重组。
- 回滚边界：删除或不填写建筑名/设施名分类即可保持普通术语流程；不得根据中文长度自动猜建筑名，也不得把软预算升级为无例外 hard blocker。
- 剩余风险：正式名与地图短标签仍依赖产品提供独立字段；没有独立字段时只能在语义完整、自然和紧凑之间人工取舍。
- 必读文档：`docs/UI_NAME_TRANSLATION_STANDARD.md`、`docs/quality-harness.md`

## 2026-07-13 大文本多语言工作流 V2

- 状态：`validated`
- 代码版本：`3f816ed`，分支 `codex/large-text-workflow-hardening`
- 触发问题：多 workbook、多语言任务存在重复源文重复调用、失败后整批重跑、深校建议直接影响最终译文、XLSX 多次保存后才暴露结构问题，以及只检查目标单元格非空导致的假通过风险。
- 实施改动：增加线性只读分包、历史交付和精确术语复用、唯一文本去重、API 并发和字符预算、失败拆包、按模型/供应端/语言隔离的 checkpoint、深校建议 checkpoint、主控二次审计、缓存级 QA、精确 XLSX XML 写回、ID/key/源文四重定位、样式索引检查和最终缓存逐格读回。
- 审查返工：提交前代码审查发现并修复 1 个 Critical 和 5 个 Important 问题，包括 `QA摘要.xlsx` 同名覆盖、切换供应端或审校器错误复用 checkpoint、worksheet 命名空间丢失、重复源文错行和读回只查非空不查值。
- 验收证据：基线为 184 项测试；最终 `python -m unittest discover -s tests -p 'test_*.py'` 为 206 项全部通过。真实 `7.13新增.xlsx + 7.13UI新增.xlsx` 只读分包得到 267 行、145 条唯一文本、2670 个目标单元格，分包耗时 0.55 秒。
- 生效范围：目标语言超过 4 个、多 workbook、唯一文本超过 5,000 条，或用户明确要求全量逐句/深度校对的大文本任务。
- 默认入口：`python scripts\run_large_text_multilingual_runner.py run ...`
- 回滚边界：普通单语言小表继续使用原 translation/quality harness；如 V2 runner 失败，不允许跳过缓存门禁后手工覆盖 workbook，只能保留 `_work` checkpoint 后修复或回到原小表工作流重新执行。
- 剩余风险：当前提交尚未合并到 `main`、尚未推送，也未同步到下游 studio；其他线程必须确认当前分支或后续合并版本包含本条记录后再使用 V2。
- 必读文档：`docs/LARGE_TEXT_MULTILINGUAL_WORKFLOW_V2.md`

## 2026-07-14 检索优先的唯一文本逐语言深校契约

- 状态：`validated`
- 代码版本：文档基线 `3ace84a`；本次为执行契约更新，无生产代码改动。
- 触发问题：小型多语言表存在大量重复行；如果逐行重复翻译/审校会浪费时间，而直接复用历史译文又可能保留旧语病。原 `AGENTS.md` 还要求用户另行明确授权 subagent，与“用户明确触发深校后按语言审校”的 handoff 约定不一致。
- 实施改动：固定为“精确历史复用 -> 精确术语复用 -> 模型补译 -> 唯一文本去重 -> 按语言 reviewer 建议 -> 主控二次纠偏 -> 稳定键扩展写回 -> 结构与客户端读回”；明确深校触发即授权按语言 reviewer subagent，且 subagent 不得直接写交付文件。
- 验收证据：真实匿名单 workbook 任务共 257 个源行、104 条唯一文本、2 个目标语言、514 个目标单元格；49 条复用精确术语/历史，55 条模型补译；两语完整审校提出 40 项建议，主控回退 3 项并补充 3 项纠偏，最终保留 40 项修改。最终空译文 0、中文残留 0、重复冲突 0、hard blocker 0，源列未改，交付目录仅含成品和 `QA摘要.xlsx`，Excel 原生只读打开成功。本次无生产代码改动，因此未新增代码回归测试。
- 生效范围：所有需要术语/历史检索的 workbook 翻译任务；用户明确触发深校时，普通 harness 和大文本 V2 都必须采用相同的“建议与最终写回分离”审计契约。
- 回滚边界：没有精确历史或术语时允许直接进入模型补译，但仍须唯一文本去重、逐语言审校和主控纠偏；不得回退为按重复源行多次调用模型或让 subagent 直接写 workbook。
- 剩余风险：短分类名可能缺少上下文，主控仍需结合英文参考、项目 brief 和相邻内容判断；本次任务目录中的临时脚本不是公共 API，不得跨项目复制为正式 harness。
- 必读文档：`docs/workflow-execution-thread-handoff.md`

## 2026-07-21 公告双 Sheet 句子级术语适配

- 状态：`validated`
- 触发问题：公告术语交付表新增 `Glossary + SentenceTemplates` 格式，旧 harness 只读活动 Sheet，句子级官方表达完全不进入 workpack；同时真实译文回放暴露中文月份数字、全半角括号和术语空格/连字符的 8 个误报。
- 实施改动：新增可选 `SentenceTemplates` 严格解析、`official_exact/official_similar` 分级检索、`<@数字>` 动态占位符匹配、`sentence_adaptations_json` 中转列和逐语言 workpack 证据；精确句覆盖范围可采用官方自然词形，相似句只作参考。受保护 token 改为 Unicode/括号等价比较，中文月份数字允许月份名称本地化，术语空格和连字符按等价形式比较；`import-ai` 失败会生成逐项 QA JSON 并在重试前清理旧报告。
- 验收证据：单元测试先出现 5 个预期失败，再实现通过；真实匿名公告回放为 23 个非空段落、4 个目标语言、92 个译文，词级命中 17 行、句子级适配命中 16 行、`official_exact` 1 次、`official_similar` 25 次。完整执行 `prepare -> import-ai -> apply -> deliver`，hard blocker 0，生成 4 个 DOCX 和 1 个 `QA摘要.xlsx`，交付目录无过程文件。
- 生效范围：所有通过 `scripts/run_announcement_docx_harness.py` 执行的公告 `.docx/.txt` 任务；旧单 Sheet 术语表和旧中转表继续兼容。
- 回滚边界：删除 `SentenceTemplates` 可回到纯词级检索；不得回退为把 `official_similar` 示例整句复制到当前公告，也不得恢复月份数字、全半角括号和连字符的机械误报。
- 剩余风险：句子级语义一致性仍由模型翻译和人工/深校判断，机器 QA 只验证结构、受保护内容和可确定的术语约束；不把任何客户术语表或任务目录脚本提交公共仓库。
- 必读文档：`docs/workflow-harness-context.md`、`docs/workflow-execution-thread-handoff.md`

## 2026-07-24 技能名与地名 UI 专名策略

- 状态：`validated`
- 代码版本：本次提交（基于 `main` 的 `51cf374`）
- 触发问题：真实移动端 UI 技能名筛选中，63 条候选有 25 条需要从三词以上压缩为两词以内；原流程只有通用短文本预算，不能区分技能名、地名和描述句，也不能发现不同中文专名被压成同一目标译名。
- 实施改动：术语加载保留 `分类/category/type` 并派生 `name_type`；只有显式技能名/地名类别才启用策略。workpack 和 AI 审校提示新增 `NAME` 元数据；英语技能名采用两词 / 24 字符软预算，地名采用两个核心词 / 28 字符软预算，其他语言按约两个核心语义单位自然表达。机审和最终 workbook 扫描新增超长与撞名预警，不做机械截断或自动覆盖。
- 验收证据：先运行定向测试确认 `utils.name_policy` 缺失、workpack 无专名字段、AI prompt 无规则共 3 类预期失败；实现后定向 8 项通过。`python -m unittest discover -s tests` 共 226 项通过；`python scripts\run_quality_harness.py fixtures\quality_regression.json` 共 67 个 fixture 全部通过，含技能名/地名好坏例和两条撞名回归；语法编译检查通过。
- 生效范围：普通 translation harness、`process_language` 基础/深度审校、最终 `quality_harness` workbook 扫描，以及项目提示词模板。
- 回滚边界：删除或不填写术语表专名分类即可保持原通用短文本流程；不得改成根据中文长度自动猜技能名/地名，也不得把软预算升级为无例外 hard blocker。
- 剩余风险：机器只能检查英文表面长度和重复目标名，不能自动判断意象、玩法差异或世界观专名是否保留；超预算项和撞名项仍需模型结合项目 brief、上下文和既有术语逐项裁决。
- 必读文档：`docs/UI_NAME_TRANSLATION_STANDARD.md`、`docs/workflow-execution-thread-handoff.md`

## 2026-07-24 中英双源翻译与校对

- 状态：`validated`
- 代码版本：本次提交（基于 `main` 的 `51cf374`）
- 触发问题：多语言正文和技能/地名翻译需要利用已校对英语稳定术语与表达，但旧标准 harness、AI 审校和大文本 V2 只传中文；如果直接把英语替换成原文，又会丢失中文条件、数字、占位符和术语回查，并可能跨来源误用旧缓存。
- 实施改动：新增显式 `cn / cn+en / en` 三种源模式。`cn+en` 以中文为主、英语为参考，缺失英语逐行回退中文；`en` 以英语为主并用中文回查，要求英语 100% 可用。标准 workpack、AI prompt、严格审校指纹、翻译缓存、workspace runner、大文本分包、API 唯一文本签名、checkpoint 和深校请求统一携带 `translation_source/source_mode/reference_en/reference_en_status`。
- 验收证据：测试先确认标准 prepare 不接受 `source_mode`、RowState 无参考字段、AI prompt 无双源规则，以及大文本未按英语参考区分唯一请求等预期失败；实现后相关模块回归通过。`python -m unittest discover -s tests` 共 239 项通过；`python scripts\run_quality_harness.py fixtures\quality_regression.json` 共 67 个 fixture 通过；标准小表与大文本等价端到端测试均覆盖英语完整、部分缺失、英语主源拒绝、缓存隔离、英语参考内容变化失效、英语参考漂移和 API 请求签名；CLI help 与语法编译检查通过。
- 生效范围：非英语标准全量翻译、已有译文 AI 审校、workspace runner 和大文本多语言 V2；默认 `cn` 行为保持不变。
- 回滚边界：不传 `--source-mode` 即回到中文单源；不得改成发现英语列后自动切换，也不得允许 `en` 模式在英语不完整时静默回退。
- 剩余风险：机器只能判断英语非空且无中文种子，不能证明英语语义已经人工验收；选择 `en` 前仍需项目负责人确认英语质量。中英实质冲突仍需模型结合 brief、术语表和上下文裁决。
- 必读文档：`docs/BILINGUAL_SOURCE_REFERENCE_WORKFLOW.md`、`docs/workflow-execution-thread-handoff.md`、`docs/LARGE_TEXT_MULTILINGUAL_WORKFLOW_V2.md`

## 2026-09-07 完整语言包扩诊的范围约束与数字词边界

- 状态：`validated`，限定维护源诊断入口与数字解析；不是整包语义发布验收。
- 代码版本：`main/cb74fa9` 上本地未提交改动；保留此前17项未提交内容，不提交、不push、不同步Studio。
- 触发问题：目的性抽样的缺陷扣分指数被理解为整体质量；独立物理行键掩盖同sheet资源ID冲突；数字后越语/德语词首B、M加重音被当作数量单位，产生真实数字缺失误报。
- 实施改动：新增 `run_quality_diagnostics.py`，按实际单元格去重覆盖、拒绝超范围覆盖、分离未决项，报告资源ID冲突而不生成整体质量分；数字单位排除后接拉丁扩展字母及组合重音的单词，保留合法K/M/B和中文相邻单位。规范补充源版本冲突边界与非对白UI上下文证据。
- 验收证据：新测试先复现模块缺失与12 Bäume被解析成120亿，修复后pytest313 passed/77 subtests、unittest304通过；既有69项quality fixture通过，新增数字/诊断fixture由单元测试驱动，git diff --check通过。真实完整表4770行/38160目标格回放，2处数字误报消失；10组资源ID冲突被保留报告，范围检查不返回整体质量分。修订缓存cache-lint为0，但项目资源身份门禁仍为10，未伪装整包发布通过。
- 生效范围：维护源Agent入口；私有项目通过自身harness调用，不把客户案例和文本加入公共仓库。
- 回滚边界：撤销本轮范围检查模块及数字词边界改动即可，不覆盖较早未提交修复；不能以删命中或关闭QA来归零。
- 剩余风险：未同步Studio数字解析副本，backend parity未验证；目的性扩审仍不能证明全表语义无误。pytest包含既有工作表扩展及Windows临时目录清理警告，未对无关目录作清理。
- 必读文档：`docs/INDEPENDENT_TRANSLATION_QUALITY_EVALUATION.md`、`AGENTS.md`、`docs/workflow-execution-thread-handoff.md`。

## 2026-09-08 全角与客户端标点兼容门禁

- 状态：`validated`，维护源已实现并通过真实表回放及修复交付；本次提交仅包含维护源，未同步 Studio。
- 代码版本：基于 `main/db23700` 的本次标点门禁提交。
- 触发问题：旧 `fullwidth_punctuation` 只枚举少量中式标点，英语弯撇号 U+2019 和全角撇号 U+FF07 均不命中；游戏客户端把弯撇号显示成异常宽间距。
- 实施改动：增加完整 Unicode 全角/CJK 标点检查；英语默认增加 ASCII 客户端兼容策略，拦截弯引号、Unicode 破折号和省略号。项目可用 `--punctuation-mode ascii|typographic` 显式覆盖；workbook、缓存和最终读回门禁共用同一规则。自动修复仅转换明确标点映射，并跳过 JSON、标签属性、变量和程序 token。
- 验收证据：固定回归包含直引号好例、弯引号坏例、全角字母数字符号坏例、西葡/法语重音好例及受保护结构好例。真实9.7英语表只读扫描9704个非空文本，检出31个单元格，其中2个含全角/CJK字符、29个含不兼容标点；修复词条逐项读回后标点阻断0，ID和中文一致，单词、数字、占位符及标签保持。最终复验为 pytest 328 passed、77 subtests passed，fixture 87项通过，`git diff --check`通过。
- 生效范围：维护源 workbook harness、表面修复、large-text `cache-lint` 与 `readback-gate`。非英语默认不强制 ASCII，避免破坏合法排印标点及重音字母。
- 补充交付验证：西语和葡语各扫描9701个非空字符串，分别修复9条和16条，以两个三列页签独立交付。标点阻断0；全部ID、中文、单词、重音字母、倒问号/倒感叹号、数字和程序标记保留；源表哈希不变，Excel普通模式打开并逐格读回一致，发布门禁通过。ASCII仅约束明确标点映射，不是去除所有非ASCII字符，也不增加模型调用。
- 回滚边界：撤销 `utils/punctuation_policy.py` 及三处调用即可；不得退回用“非 ASCII 一律删除”的破坏性策略。
- 剩余风险：客户端异常还可能受字体 fallback 或排版引擎影响；本轮只保证文本字符符合 ASCII 项目策略，仍需游戏内复测。客户原始工作簿未覆盖，单独交付修复词条。
- 必读文档：`docs/quality-harness.md`、`AGENTS.md`、`docs/workflow-execution-thread-handoff.md`。

## 2026-09-07 改名旧译与跨行系列复核

- 状态：`validated`，限定维护源检查能力，不代表真实整表重新深校或发布通过。
- 代码版本：核心检查已包含在 `main/f8aee2e`；本条所在后续提交补齐等级数字歧义防误报与执行规范。用户已授权提交推送，远端完成状态以实际读回为准；未同步 Studio。
- 触发问题：项目临时 QA 漏查正文改名和同组等级句式；术语姓名分类存于备注，旧加载器不识别，导致旧名称被继续复用。
- 实施改动：普通 QA 与大文本分包共享明确备注分类兼容；正式 workbook harness 新增罗马序号/同条件等级系列检查，以及显式 `--history` 异源同译候选。分组不跨 sheet，不按多数自动改写，等级数值歧义跳过格式推断；历史候选只警告，需语境裁决。最终成品必须调用正式 harness 并显式指定用户给出的最新术语表。
- 验收证据：姓名备注测试先复现漏检，修复后 pytest 322 passed、77 subtests passed；质量 fixture 78 项通过，涵盖错误系列、正确系列、改名旧译、已修正译文、自然共享短按钮及分类好坏例。真实三语前后只读回放中，旧表每语命中目标问题候选22项，修正表目标错误0；每语仍有1项历史“以上/及以上”候选，经当前条件语义裁决保留译文。最终成品309行每语的正式扫描加历史检索耗时1.002至1.059秒，无模型调用。`git diff --check` 通过。
- 生效范围：维护源 Agent 最终 workbook harness；备注分类同时进入大文本 workpack。未同步的 Studio 入口不自动获得这些检查。
- 回滚边界：可撤销本轮检查模块、CLI 可选参数及备注分类兼容，不覆盖此前工作。禁止通过删除警告、自动扩大同义替换或关闭姓名检查来凑零。
- 剩余风险：真实表另有普通修饰词被术语备注标为姓名，产生每语1个原始阻断，已记录语境裁决而未改用户术语表；全表历史候选尚不能等同确认错误或语义放行。pytest 有既有工作表扩展及 Windows 临时目录清理警告。未修改客户交付文件，未执行新一轮全量深校。
- 必读文档：`docs/quality-harness.md`、`AGENTS.md`、`docs/workflow-execution-thread-handoff.md`。

## 2026-09-07 维护源提交发布核验

- 授权范围：用户本轮明确要求推送工作流改动，覆盖此前“仅本地、不提交、不push”的提交限制；仍不包含Studio同步、项目专用harness、译文及审计交付。
- 代码版本：本条所在提交，基于 `main/cb74fa9`；此前未推送的小批量历史检索提交随维护源主分支一并发布。历史条目的状态保留为当时快照，以本条说明当前发布范围。
- 发布内容：批准姓名快照及精确分类、对白话轮和可信审计输入、已知语义回归拦截、系列文案一致性与显式历史复用预警、诊断覆盖与资源ID冲突检查、数字单位词边界，以及配套规范和通用回归用例。
- 本轮验证：`py -3 -X utf8 -m pytest -q` 为320 passed、77 subtests passed；`scripts/run_quality_harness.py fixtures/quality_regression.json` 为78个用例全部通过；`git diff --check`通过。存在既有工作表扩展与Windows临时目录清理警告，无测试失败。
- 发布边界：只提交维护源通用代码、测试、fixture与文档；不新增客户工作簿、项目名称、交付路径或密钥。远端发布是否完成，以推送后的远端分支读回为准。
- 剩余风险：未同步Studio，未运行Studio backend parity；本轮未重新调用外部模型或重跑真实语言包翻译，不能把代码回归通过等同于所有语言语义正确。

## 2026-09-09 备注修改词条的全量语义复核契约

- 状态：`validated`，限定维护源 Agent 执行规则与本次真实批次验收；未修改通用执行器，未提交、未推送、未同步 Studio。
- 触发问题：源文改名或删词后，同 ID 与历史整句不能证明旧译仍正确；只审建议修改集合会漏掉错误的保留项。
- 实施改动：新增 `docs/MODIFIED_SOURCE_REVIEW.md`，在 AGENTS 与执行交接中要求所有修改行逐格复核当前源文语义；显式重译排除旧译、旧英语参考、历史句库及旧任务缓存。首轮 KEEP 也进入第二轮，所有最终译文绑定源文和译文指纹；ID 仅定位。
- 验收证据：匿名真实批次 299 行、897 个三语目标格完成独立重译和两轮全量语义记录。首轮建议 231 格；二轮建议 73 格，经主控保留 50、拒绝 14、改写 9。最后相对旧表变动 880 格并标黄，17 格独立重译后文字相同。结构及缓存未决阻断为 0；三个正常大小写词形误报逐格裁决，原始报告保留。缺一审、旧源文指纹、缺二审、缺最终结论四项负向检查均能阻断。
- 交付证据：Microsoft Excel 普通模式逐格读回 1860 格，目标缓存匹配、非目标内容与样式保留、源文件哈希不变，正常打开无差异。QA 摘要也普通打开并回读指标；输出编码及 XLSX 路径/XML 门禁通过。客户文本、脚本和证据全部保留仓外。
- 生效与回滚：这是 Agent 执行契约，不宣称历史临时脚本、通用引擎或其他仓库已自动实现。撤回仅涉及本次规则段和新文档，不覆盖既有未提交改动。结构门禁通过不代表免除语义判断；相邻语境有效，不因机械追求术语字面命中破坏自然表达。

## 2026-09-09 显式数量与未决审校阻断

- 状态：`validated`，限定本地维护源的确定性数量规则与明确未决字段/交接语句检测。
- 代码版本：`ec142ac` 基础上的本地工作区修改，未提交、未push、未同步工作台；保留其他任务改动。
- 触发问题：真实技能描述缺攻击次数/每次伤害；多个道具旧译缺时长，术语审校已登记但继续放行。旧小数字豁免、只比较新旧译文数字以及未决备注未阻断共同漏检。
- 实施改动：新增共享 quantity_guard 和 review_findings；接入 cache-lint、正式 workbook harness、review 输入验收，并补审校提示及契约。
- 验收证据：全仓353项测试和146个子案例通过，数量专项63例；真实29,037格只读规则扫描，9格确认遗漏；旧门禁0/9拦截，新门禁9/9；修复词条三语harness及cache-lint无阻断；Excel普通模式96格回读无差异。
- 生效范围：CN至EN/ES/PT的显式小时/分钟/秒/天/次数与每次伤害表达；结构化未决字段及明确交接语句。
- 回滚边界：回退本条新增模块与接入、对应fixture/测试和提示增量，不回退其他未提交修改及客户成品。
- 剩余风险：有限规则不等于全语义正确；复杂单位换算和其他语言需语义判断；外部脚本丢弃元数据会失去该部分检查，必须按契约导入；产品镜像尚未同步。
- 必读文档：`QUANTITY_AND_UNRESOLVED_REVIEW_GATE.md`、`MODIFIED_SOURCE_REVIEW.md`。


## 2026-09-09 数字语义、KEEP 二审及验收完整性修复

- 状态：`validated`，限定本地维护源；代码版本为 `ec142ac` 加本轮未提交补丁，未提交、未推送、未同步 Studio。保留工作区其他改动。
- 问题及证据：上一补丁只有次数/时长，通用数字仍豁免 0～10；KEEP 实际没进二审；audit ACCEPT 未决、变量次数损失、额外变量、空缓存/目录和错误成品回读等 8 个坏输入均能误过门禁。实际数据另确认编号遗漏和色值漂移，客户文本留仓外。
- 改动：共享源译完整性检查覆盖可读小数字、编号实体、明确攻防/敌方/短句否定、变量次数与标签及换行；实际回读复用检查并阻断空覆盖/无源列/歧义目标列；全 KEEP/FIX 独立二审、KEEP 可二审纠正但不得暗改或 REVERT 绕过，未决 audit 阻断、检查点版本失效；试写拒绝覆盖源文件。
- 验证：全仓 364 passed、157 subtests passed（25 warnings），正式 fixture 164 例通过。8 个旧误放行输入现全部阻断，匿名 KEEP→REVISE→最终缓存端到端通过。真实主表 29,037 格专项扫描，累计17行21格修复后有限规则残留0；51格缓存/成品与三语harness无阻断，两份XLSX门禁通过，Excel普通打开150格回读0差异，源表哈希不变。
- 生效与回滚：本地 gate、harness、proofread、提示和契约；回退仅限本轮增量，保留此前数量模块和其他任务改动，不回写客户源表。不可通过忽略未决记录或删坏输入来回退。
- 剩余边界：未覆盖任意自然语言等价关系；本次是全主表规则扫描和候选复核，不是全表深度逐句审校；未列语言和外部丢弃元数据脚本仍需专项处理。测试警告不影响断言通过；工作台镜像未同步。
- 必读文档：QUANTITY_AND_UNRESOLVED_REVIEW_GATE.md、MODIFIED_SOURCE_REVIEW.md、AGENTS.md、workflow-execution-thread-handoff.md。


## 2026-09-09 流程修复发布验收

- 用户已授权提交并推送本轮翻译 QA 修复。仅选择备注修改规则、数量/语义门禁、KEEP 二审、未决阻断和成品验收相关的 21 个文件；竞品建议、术语完善和初译缓存优化保留在原工作区。
- 从暂存区导出独立发布副本复测：342 passed、157 subtests passed（26 warnings）；正式质量 fixture 164 例通过。该数字与此前含其他任务改动的工作区 364 项不同，发布副本没有依赖未提交的其他功能。
- 新增内容经客户信息/密钥检查、UTF-8 输出门禁及 git diff --cached --check 验证。维护源 origin/main 为发布目标；Studio 同步及 backend parity 不在本次推送范围。远端是否成功以提交后的远端分支读回为准。
