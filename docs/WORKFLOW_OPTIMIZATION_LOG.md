...
## 2026-09-07 定向修复的姓名快照、话轮证据与已知语义回归

- 状态：`validated`，限定为显式姓名快照、证据传输、精确语境豁免及已知误译回归；不是模型全部语义能力改善证明。
- 代码版本：`main/cb74fa9` 上本地工作区改动；用户明确要求只做本地修复，暂不提交、不同步，未push。
- 触发问题：同表批准的新姓名未纳入外部术语约束；审计缺少目标先行词；自然代词回指会被新姓名硬规则误报；模型多次将英语习语的担忧对象错译仍判ACCEPT。
- 实施改动：增加显式姓名行到私有JSON快照的入口，记录来源哈希并阻断冲突/缺语种/越界；pack和cache-lint共用快照。显式对白场景自动补齐前后源文及当前目标话轮，签名包含证据，单语言请求只携带该语言目标。单格term_waivers绑定源文、译文和语境。对白审计增加目标盲读、语义字段校验与checkpoint隔离；已知法语担忧对象错位以源条件化窄规则阻断，并在审计和缓存双处复核。
- 验收证据：新增测试先复现缺失快照加载、场景证据签名不变、接受缺语义证据等失败；全仓302 passed、77 subtests passed，既有69项质量fixture通过，新增4组语义好坏例由测试另行驱动，git diff --check通过。真实定向任务复核123格，修改96格；82个原确认问题全部修正，含48处错名；537行/4,296目标格最终cache-lint与readback为0，实际修改集合等于黄底集合，原件哈希不变。
- 真实模型验证：最终3个已知坏建议均回退，2个正确对照保留；其中2个性别问题由模型回退，法语习语由确定性回归兜底。未经回归规则时模型仍会误判，不把新增盲读或格式字段声称为语义自动闭环。
- 生效范围：维护源Agent执行入口。已有工作台镜像和旧交付不自动更新；复杂长场景超过48轮只给前后各8轮并标记不完整，未知人物关系仍须人工补证。
- 回滚边界：可撤销新增快照/场景证据/盲读/窄规则，但不能通过全局关闭姓名QA、接受缺证据修改或把正确代词当姓名变体绕过错误。词形豁免只属于已核实的一格。
- 剩余风险：自然语言未知错误仍可能漏检，盲读每个含对白修改的审计批次增加一次调用；6个场景或人物信息不足项保留待证，不列为确认错误。未做Studio同步或backend验证，未重跑全表语义评分。
- 必读文档：`docs/INDEPENDENT_TRANSLATION_QUALITY_EVALUATION.md`、`AGENTS.md`、`docs/workflow-execution-thread-handoff.md`、`docs/LARGE_TEXT_MULTILINGUAL_WORKFLOW_V2.md`

## 2026-09-07 独立评估后的角色约束与审计证据传递

- 状态：`validated`，限定为输入契约、确定性角色约束及回归验证；不表示语义错误已经消除。
- 代码版本：`main` 的 `cb74fa9` 基线上的本地未提交改动；用户要求不提交、不 push，尚未同步 Studio 执行副本。
- 触发问题：真实八语成品的术语表使用“角色”分类，而大文本加载器仅识别“角色名”等标签，强名称约束为 0；二次审计调用链又只传 review_key、语言、建议和理由，没有原译、源文和语境。独立评估确认了深校引入的性别、习语退化。
- 实施改动：补齐精确角色分类别名，保留普通亲属称谓的语境弹性；英语主源不补回英语已省略的中文姓名。主控按可信 review_rows 给 auditor 补齐源模式、原译、源文、上下文、token、术语和风险标签；reviewer 自报的 current/context 不能覆盖这些证据。加强性别/称谓证据要求，并隔离新版审校和审计 checkpoint。新增独立评估规范，要求同批批准姓名先纳入临时术语快照、对白按完整话轮复核、缺陷与偏好分开统计。
- 验收证据：角色别名测试先出现 8 个预期失败；审计调用链测试先复现缺少 current 的 KeyError，修复后通过。全仓 `py -3 -X utf8 -m pytest -q` 为 293 passed、77 subtests passed；既有质量 fixture 69 项通过；`git diff --check` 通过。真实 537 行、4,296 格只读回放启用 31 条专名约束，当前术语范围误报 0；回放一处历史错名准确检出 1 个 term_missing。正式成品、源表、术语表与 QA 的哈希不变。
- 语义验证边界：三条已知退化做真实模型回放，性别两条分别 REVISE / REVERT，但法语习语一条仍错误 ACCEPT。只能说明证据传递缺陷已修复，不能宣称模型语义回归全通过，也不能以 646 个修改之类的工作量代替质量收益。
- 生效范围：维护源大文本 pack/cache-lint 和 run_deep_proofread 的下一次执行；现有交付与缓存未改。源模式、术语元数据及 checkpoint 变化会使旧快照需要重新核对。
- 回滚边界：按本次差异撤回分类、匹配和审计补证据改动；不得通过清空术语或把猜测性性别修改改成自动接受来规避失败。
- 剩余风险：同批新姓名未在外部术语表时，仍须按规范先合并临时名称基线；说话人和关系证据不齐时模型可能仍误判。Studio 同步副本存在本次任务之前的多文件差异，本次只更新入口说明，未把无关历史改动全量镜像。
- 必读文档：`docs/INDEPENDENT_TRANSLATION_QUALITY_EVALUATION.md`、`AGENTS.md`、`docs/workflow-execution-thread-handoff.md`、`docs/LARGE_TEXT_MULTILINGUAL_WORKFLOW_V2.md`

## 2026-09-02 小批量历史语言表精确检索

- 状态：`validated`
- 代码版本：本次提交，基于 `main`
- 触发问题：真实 6 条、9 语言邮件任务为了确认历史官方句式，最初尝试直接导入 1.4 MB 完整语言表并出现 30 秒超时；该行为容易被误解为重新翻译或校对整份语言包，也没有必要。
- 实施改动：新增 `scripts/run_history_lookup.py` 和 `utils/history_lookup.py`。小批量任务先加载术语表，再用当前中文原文做精确查询；历史 workbook 使用 `openpyxl read_only=True` 流式读取，只取中文列与请求语言列，全部命中后立即停止打开后续历史文件；未命中不做模糊或相似句扩展。同步更新 `AGENTS.md`、`工作流说明.md` 和线程 handoff，明确禁止小任务导入、渲染、翻译、校对或另存整份历史语言表。
- 验收证据：新增 5 项单元测试，覆盖源文去重、指定语言列、中文简称表头、拒绝标点近似匹配、全部命中后短路后续文件；真实 6 行邮件表提取为 5 条唯一中文，对 4,273 行历史语言资产进行九语精确只读查询耗时 0.662 秒，精确命中 0 后正常结束。全仓 `py -3 -X utf8 -m pytest -q` 为 290 passed、64 subtests passed；CLI help、语法编译和 `git diff --check` 通过。
- 生效范围：当前任务唯一中文不超过 200 条，且历史已验收译文仅用于精确复用的 XLSX 小批量翻译任务。
- 回滚边界：不传历史 workbook 即保持术语表加模型翻译；用户明确要求全量历史审计或任务满足大文本 V2 条件时仍走对应完整流程。不得回退为发现完整语言表后默认整本导入。
- 剩余风险：没有精确命中的文本不会获得相似句参考；如项目确实需要相似句证据，应由用户明确要求或使用项目专用句子模板，不能在小批量默认流程中静默扩大检索范围。
- 必读文档：`AGENTS.md`、`docs/workflow-execution-thread-handoff.md`、`工作流说明.md`

## 2026-08-25 结构化长文本程序托管与失败项局部修复

- 状态：`validated`
- 代码版本：工作区待提交，分支 `agent/harden-large-text-terminology-gates`
- 触发问题：真实七语任务的西语初译曾在 44 行中产生 210 个结构 blocker，集中于 3 条带大量标签/JSON 的长文本；另外 11 条只有章节序号或已知术语中文残留。旧流程把整段结构交给模型保留和重建，并在少量失败时整批重试，既慢又可能推翻已通过译文。
- 实施改动：新增程序托管模板，将有效 JSON 或含至少 3 个技术标签的中文源按 JSON 路径和标签边界拆成纯文本槽；翻译、reviewer 和 auditor 只处理纯文本，主控按原路径和精确标签序列重建。初译后新增保守本地残留修复，只处理该行命中术语和章节序号；其余残留继续由 `cache-lint` 阻断。翻译和深校 checkpoint 版本升级，避免错误复用旧整段结果。
- 验收证据：新增回归覆盖模型请求不含结构、JSON 形状与标签序列精确重建、已通过单元格不变、仅修已知残留、深校与审计不接触结构，以及 Excel 准备到交付读回的等价端到端流程；定向 40 项通过，全仓 `py -3 -X utf8 -m pytest -q` 为 281 passed、64 subtests passed，`git diff --check` 通过。
- 生效范围：大文本 V2 中 `source_mode=cn` 且源文为有效 JSON，或含至少 3 个 `<@n>`/HTML 类技术标签的行；普通文本、英文主源和历史精确复用路径不变。
- 回滚边界：删除结构化模板分流与本地残留修复调用可回到旧路径；不得在新路径失败时降级为让模型整段保留/重建结构，也不得重译已通过单元格。
- 剩余风险：无效 JSON 且技术标签少于 3 个的短文本仍走普通路径；本地残留修复只识别术语命中和章节序号，其他中文正文会继续 hard fail，需要人工确定纯文本修复内容。
...
## 2026-09-07 完整语言包扩诊的范围约束与数字词边界

- 状态：`validated`，限定维护源诊断入口与数字解析；不是整包语义发布验收。
- 代码版本：`main/cb74fa9` 上本地未提交改动；保留此前17项未提交内容，不提交、不push、不同步Studio。
- 触发问题：目的性抽样的缺陷扣分指数被理解为整体质量；独立物理行键掩盖同sheet资源ID冲突；数字后越语/德语词首B、M加重音被当作数量单位，产生真实数字缺失误报。
- 实施改动：新增 `run_quality_diagnostics.py`，按实际单元格去重覆盖、拒绝超范围覆盖、分离未决项，报告资源ID冲突而不生成整体质量分；数字单位排除后接拉丁扩展字母及组合重音的单词，保留合法K/M/B和中文相邻单位。规范补充源版本冲突边界与非对白UI上下文证据。
- 验收证据：新测试先复现模块缺失与12 Bäume被解析成120亿，修复后pytest313 passed/77 subtests、unittest304通过；既有69项quality fixture通过，新增数字/诊断fixture由单元测试驱动，git diff --check通过。真实完整表4770行/38160目标格回放，2处数字误报消失；10组资源ID冲突被保留报告，范围检查不返回整体质量分。修订缓存cache-lint为0，但项目资源身份门禁仍为10，未伪装整包发布通过。
- 生效范围：维护源Agent入口；私有项目通过自身harness调用，不把客户案例和文本加入公共仓库。
- 回滚边界：撤销本轮范围检查模块及数字词边界改动即可，不覆盖较早未提交修复；不能以删命中或关闭QA来归零。
...
- 代码版本：核心检查已包含在 `main/f8aee2e`；本条所在后续提交补齐等级数字歧义防误报与执行规范。用户已授权提交推送，远端完成状态以实际读回为准；未同步 Studio。
- 触发问题：项目临时 QA 漏查正文改名和同组等级句式；术语姓名分类存于备注，旧加载器不识别，导致旧名称被继续复用。
- 实施改动：普通 QA 与大文本分包共享明确备注分类兼容；正式 workbook harness 新增罗马序号/同条件等级系列检查，以及显式 `--history` 异源同译候选。分组不跨 sheet，不按多数自动改写，等级数值歧义跳过格式推断；历史候选只警告，需语境裁决。最终成品必须调用正式 harness 并显式指定用户给出的最新术语表。
- 验收证据：姓名备注测试先复现漏检，修复后 pytest 322 passed、77 subtests passed；质量 fixture 78 项通过，涵盖错误系列、正确系列、改名旧译、已修正译文、自然共享短按钮及分类好坏例。真实三语前后只读回放中，旧表每语命中目标问题候选22项，修正表目标错误0；每语仍有1项历史“以上/及以上”候选，经当前条件语义裁决保留译文。最终成品309行每语的正式扫描加历史检索耗时1.002至1.059秒，无模型调用。`git diff --check` 通过。
- 生效范围：维护源 Agent 最终 workbook harness；备注分类同时进入大文本 workpack。未同步的 Studio 入口不自动获得这些检查。
- 回滚边界：可撤销本轮检查模块、CLI 可选参数及备注分类兼容，不覆盖此前工作。禁止通过删除警告、自动扩大同义替换或关闭姓名检查来凑零。
- 剩余风险：真实表另有普通修饰词被术语备注标为姓名，产生每语1个原始阻断，已记录语境裁决而未改用户术语表；全表历史候选尚不能等同确认错误或语义放行。pytest 有既有工作表扩展及 Windows 临时目录清理警告。未修改客户交付文件，未执行新一轮全量深校。
- 必读文档：`docs/quality-harness.md`、`AGENTS.md`、`docs/workflow-execution-thread-handoff.md`。

## 2026-09-07 维护源提交发布核验

- 授权范围：用户本轮明确要求推送工作流改动，覆盖此前“仅本地、不提交、不push”的提交限制；仍不包含Studio同步、项目专用harness、译文及审计交付。
- 代码版本：本条所在提交，基于 `main/cb74fa9`；此前未推送的小批量历史检索提交随维护源主分支一并发布。历史条目的状态保留为当时快照，以本条说明当前发布范围。
- 发布内容：批准姓名快照及精确分类、对白话轮和可信审计输入、已知语义回归拦截、系列文案一致性与显式历史复用预警、诊断覆盖与资源ID冲突检查、数字单位词边界，以及配套规范和通用回归用例。
- 本轮验证：`py -3 -X utf8 -m pytest -q` 为320 passed、77 subtests passed；`scripts/run_quality_harness.py fixtures/quality_regression.json` 为78个用例全部通过；`git diff --check`通过。存在既有工作表扩展与Windows临时目录清理警告，无测试失败。
- 发布边界：只提交维护源通用代码、测试、fixture与文档；不新增客户工作簿、项目名称、交付路径或密钥。远端发布是否完成，以推送后的远端分支读回为准。
- 剩余风险：未同步Studio，未运行Studio backend parity；本轮未重新调用外部模型或重跑真实语言包翻译，不能把代码回归通过等同于所有语言语义正确。
