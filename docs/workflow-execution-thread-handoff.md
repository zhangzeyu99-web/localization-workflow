...
本地 cache-lint 和 workbook harness 已共享 EN/ES/PT 数量门禁；审校建议校验和 cache-lint 拦截非空 unresolved_issues 及明确待主控 QA 的旧式备注。不能以原表已有或结构无新增错误代替语义验收。项目脚本须保留审校元数据，不能只导入译文。执行 `QUANTITY_AND_UNRESOLVED_REVIEW_GATE.md`；尚未同步 Studio，不声称产品端生效。
