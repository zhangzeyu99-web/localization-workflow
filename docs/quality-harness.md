...
- 这里的 ASCII 兼容是“已知不兼容标点转为普通键盘标点”，不是文件转码，也不是只允许 ASCII 字符。比如 `Don’t` 改为 `Don't`、`“Text”` 改为 `"Text"`、`…` 改为 `...`、`＆` 改为 `&`。西葡语的 `ñ、á、ç、ã` 和 `¡¿` 保留，不删除合法语言字符。
- 拉丁字母目标语言检查 Unicode 全角字母、数字、符号及常见 CJK 标点，不再只枚举逗号、句号等少量符号。合法重音字母不是全角字符，不得删除或 ASCII 化。
- 英语默认采用客户端兼容策略，`’ ‘ “ ”`、Unicode 破折号和省略号报告 `incompatible_punctuation`；正式表面修复只替换明确映射字符，不重译句子。项目允许排印标点时可显式传 `--punctuation-mode typographic`。
- 非英语默认只做全角检查；若项目客户端明确要求 ASCII，workbook harness、`cache-lint` 和 `readback-gate` 均传 `--punctuation-mode ascii`，保证写回前后执行同一规则。
- JSON、标签属性、变量、占位符和程序 token 不直接改写。结构化内容按既有纯文本槽流程处理，修复后原位回填并核对结构。
- 这些检查和明确映射修复在本地运行，不增加模型调用或要求重译已通过的整条文本。文本门禁通过不能代替客户端字体与排版实测；未在游戏内复测时不得声称显示问题已完全解决。
